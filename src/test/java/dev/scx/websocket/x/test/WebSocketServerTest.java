...
public class WebSocketServerTest {
    ...
    public static void test1() throws IOException, InterruptedException {
        // 测试 是否存在 幽灵连接
        // 查看 localhost: 8882 eventWebSockets 最终应该为 0 , number 也应该为 0 (表示没有多次触发 onClose)
        startServer();
        ...
        for (int i = 0; i < 10000; i = i + 1) {
            ...
            Thread.ofVirtual().start(() -> {
                try {
                    var scxWebSocket = new WebSocketClient().handshakeRequest().uri("ws://localhost:8882/websocket").upgrade();
                    Thread.sleep(1000);
    ...
    public static void startServer() throws IOException {
        ...
        httpServer.onRequest(c -> {
            if (c instanceof ScxWebSocketServerHandshakeRequest wsRequest) {
