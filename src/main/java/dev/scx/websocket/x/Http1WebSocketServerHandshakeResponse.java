...
///
/// @author scx567888
public final class Http1WebSocketServerHandshakeResponse implements ScxWebSocketServerHandshakeResponse {

    private final Http1WebSocketServerHandshakeRequest request;
    private final Http1ServerResponse _response;
    private final WebSocketOptions webSocketOptions;
    private ScxWebSocket webSocket;

    public Http1WebSocketServerHandshakeResponse(Http1WebSocketServerHandshakeRequest request, Http1ServerResponse _response, WebSocketOptions webSocketOptions) {
    ...
    private void validateHandshake() throws ScxWebSocketServerHandshakeInvalidException {
        // 1, 校验 websocket 版本
        var secWebSocketVersion = request.secWebSocketVersion();
        if (secWebSocketVersion == null) {
            throw new ScxWebSocketServerHandshakeInvalidException("Missing required header: Sec-WebSocket-Version");
        }
        if (!"13".equals(secWebSocketVersion)) {
            throw new ScxWebSocketServerHandshakeInvalidException("Unsupported Sec-WebSocket-Version: " + secWebSocketVersion);
        }

        // 2, 校验 secWebSocketKey
        var secWebSocketKey = request.secWebSocketKey();
        if (secWebSocketKey == null) {
            throw new ScxWebSocketServerHandshakeInvalidException("Missing required header: Sec-WebSocket-Key");
        }
        byte[] decoded;
        try {
            decoded = Base64.getDecoder().decode(secWebSocketKey);
        } catch (Exception e) {
            throw new ScxWebSocketServerHandshakeInvalidException("Invalid Sec-WebSocket-Key: not a valid Base64 value");
        }
        if (decoded.length != 16) {
            throw new ScxWebSocketServerHandshakeInvalidException("Invalid Sec-WebSocket-Key: decoded length must be 16 bytes");
        }
    }

    @Override
    public ScxWebSocket upgrade() throws IllegalSenderStateException, ScxHttpSendException, ScxWrappedException, ScxHttpReceiveException, ScxWebSocketServerHandshakeInvalidException {
        if (webSocket == null) {
            // 先校验握手 是否正确.
            validateHandshake();
            // 实现握手接受逻辑, 返回适当的响应头
    ...
    @Override
    public Http1WebSocketServerHandshakeRequest request() {
        return request;
    }
    ...
    @Override
    public Http1WebSocketServerHandshakeResponse statusCode(ScxHttpStatusCode statusCode) {
    ...
    public Http1WebSocketServerHandshakeResponse reasonPhrase(String reasonPhrase) {
